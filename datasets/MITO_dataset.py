...
class MITO_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.dataset_type == 'mito':
        ...
        elif self.dataset_type == 'mito_copy':
            img = cv2.imread(img_path, -1)
            mask = cv2.imread(mask_path, -1)
            img_ = img_PreProc_mito(img, pro_type='clahe_norm')
            ...
            sample = {"image": image_chw,
                      "mask": mask_,
                      "ID": os.path.split(img_path)[1]}
        elif self.dataset_type == 'mito_fractal':
            ...
            sample = {"image": image_chw,
                      "mask": mask_,
                      "ID": os.path.split(img_path)[1]}
        elif self.dataset_type == 'mito_fractal_three_decoder':
            npy_path = img_path.replace("images", self.fractal_dir)
            npy_path = npy_path.replace(".tif", ".npy")
            fractal_info = np.load(npy_path)
            img_FD, img_FL = npy_PreProc(fractal_info)
            ...
            if self.train:
                ...
                sample = {"image": image_chw,
                          "mask": mask_,
                          "ID": os.path.split(img_path)[1]}
        elif self.dataset_type == 'mito_fractal_three_decoder_weighted':
            npy_path = img_path.replace("images", self.fractal_dir)
            npy_path = npy_path.replace(".tif", ".npy")
            fractal_info = np.load(npy_path)
            img_FD, img_FL = npy_PreProc(fractal_info)
