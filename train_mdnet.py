...
def train_one_epoch(epoch, model_type, total_steps, dataloader, model,
                    device, criterion_iou, criterion_bce, optimizer, lr,
    ...
    for inputs in dataloader:
        ...
        # compute loss
        loss1 = criterion_iou(pred, labels)
...
def train_eval_model(opts):
    ...
    shutil.copy("train_mdnet.py", os.path.join(model_dir, "train_mdnet.py"))
    ...
    dataloaders_dict = {
        "train": build_data_loader(dataset_name, train_data_dir, train_batch_size, dataset_type, is_train=True,
                                   fractal_dir=fractal_dir, edge_dir=edge_dir, skeleton_dir=skeleton_dir),
    ...
    # loss layer
    criterion_iou = create_criterion(criterion=loss_criterion)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        total_steps = train_one_epoch(epoch, model_type, total_steps,
                                      ...
                                      model, device,
                                      criterion_iou, criterion_bce, optimizer, lr,
                                      display_iter, log_file, warmup_step, warmup_method)
...
if __name__ == '__main__':
    ...
    model_choice = ['Two_decoder_Net', 'Multi_decoder_Net']
    date = '20240312'

    opts = dict()
    opts['dataset_type'] = 'er_fractal_three_decoder'
    ...
    log_dir = "./train_logs/" + str(opts["dataset_type"]) + str(opts["model_type"]) + "_iou_bce_loss_one_half_half" + \
