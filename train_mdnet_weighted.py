...
def eval_one_epoch(epoch, model_type, threshold, dataloader, model, device, epoch_dir, log_file):
    with torch.no_grad():
        ...
        for inputs in dataloader:
            ...
            outputs, pred_skeleton, pred_edge = model(images)
...
def train_eval_model(opts):
    ...
    shutil.copy("models/md_net.py", os.path.join(model_dir, "md_net.py"))
    shutil.copy("train_mdnet_weighted.py", os.path.join(model_dir, "train_mdnet_weighted.py"))
    ...
    dataloaders_dict = {
        "train": build_data_loader(dataset_name, train_data_dir, train_batch_size, dataset_type, is_train=True,
                                   fractal_dir=fractal_dir,weight_dir=weight_dir,edge_dir=edge_dir,skeleton_dir=skeleton_dir, log_file=train_dir, update_d=update_d),
...
if __name__ == '__main__':
    ...
    dataset_list = ['er', 'er_fractal', 'er_fractal_three_decoder', 'er_fractal_three_decoder_weighted']
    model_choice = ['Multi_decoder_Net']
    date = '20240312'

    opts = dict()
    opts['dataset_type'] = 'er_fractal_three_decoder_weighted'
    opts["dataset_name"] = 'ER'
    opts["num_epochs"] = 50
    opts["fractal_dir"] = 'Fractal_info_5'
    opts["weight_dir"] = 'Weight_5_1'
    opts["edge_dir"] = 'mask_edge'
    opts["skeleton_dir"] = 'mask_skeleton'
    ...
    opts["optimizer"] = "SGD"
    opts["model_type"] = "Multi_decoder_Net"
    ...
    log_dir = "./train_logg/" + str(
        opts["dataset_type"]) + "_train_two_iou_bce_" + opts["model_type"] + '_' + opts["loss_criterion"] + '_' + str(
        opts["train_batch_size"]) + '_' + str(opts["lr"]) + '_' + str(opts["num_epochs"]) + '_' + str(
        opts["threshold"]) + '_' + str(opts["warmup_step"]) + '_' + date + '_' + str(
        opts["weight_decay"]) + '_warmup_' + opts["fractal_dir"]
